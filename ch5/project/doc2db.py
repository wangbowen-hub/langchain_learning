...
def split_document(
    ...
    # split_documents是一个list，元素类型为langchain_core.documents.base.Document
...
def process_file_paths():
    ...
    with tqdm(total=len(FILE_PATH), desc="处理文件", ncols=80) as progress_bar:
        for path in FILE_PATH:
            # 加载文件，document是一个List，每页对应一个元素,元素类型为langchain_core.documents.base.Document
            document = load_document(path)

            file_name_with_extension = os.path.basename(document[0].metadata["source"])

            # 文档分割，返回分割后更小的langchain_core.documents.base.Document的list，并在元素的metadata字典里添加chunk_index_name字段
            ...
            # 为small_chunk_split里每个元素的metadata字典里f"{window_index_name}_lower_bound和f"{window_index_name}_upper_bound字段
            ...
            # 将small_chunk_split中CHUNK_SCALE个块合并成一个块
